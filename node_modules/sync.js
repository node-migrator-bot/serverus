...
module.exports = function(options){
    ...
    function checkout(branch, force, callback){
        var info = getCheckoutInfo(branch.name);

        if(!path.existsSync(branch.location)){
            fs.mkdirSync(branch.location, '0766');
        }

        git.log('-n1 --pretty=oneline "' + branch.fullName + '" --', function(err, output){
            if(err) return callback(err);

            var commitRef = output.split(' ')[0];
            if(info.lastCommit === commitRef && !force){
                console.log(branch.name, 'already up to date');
                callback(err, commitRef);
                return;
            }

            info.lastCommit = commitRef;
            setCheckoutInfo(branch.name, info);

            console.log('Updating', branch.name);
            checkoutTempCopy(commitRef, function(err){
                if(err) return callback(err);

                console.log('deploying', branch.name, '(' + commitRef + ')');
                rsync(serverusGitDir, branch.location, function(err){
                    if(err) return callback(err);

                    console.log(branch.name, 'deployed');
