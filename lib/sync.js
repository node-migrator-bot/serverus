...
module.exports = function(options){
    ...
    function rsync(branch, fromPath, toPath, callback){
        ...
        var gitIgnorePath = path.join(fromPath, '.gitignore'),
            gitIgnore = path.existsSync(gitIgnorePath) ? fs.readFileSync(gitIgnorePath, 'utf8').split('\n') : [],
            exclude = (branch.get('config').excludeFromDeploy || []).concat(gitIgnore),
            args = _(exclude).chain().map(function(exclude){
                    if(exclude[0] === '#' || /^\s*$/.test(exclude)){
                        return /*undefined*/;
                    }
                    return "--exclude=" + exclude;
                }).compact().unique().value()
                .concat(["--exclude=__serverus.lock", "--exclude=.git", "-rh", "--delete", fromPath, toPath]),
    ...
    function checkout(branch, force, callback){
        ...
        if(!path.existsSync(branch.get('location'))){
            fs.mkdirSync(branch.get('location'), '0766');
        }

        branch.out.write('deploy is talking:\n');
        branch.error.write('deploy is talking:\n');
